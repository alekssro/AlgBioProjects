...
echo "Obtained result:"
# Get alignment
./msa_approx.py score_matrix 5 testdata_short.txt > testdata_short_aligned.fa     
...
echo "Obtained result:"
# Get alignment
./msa_approx.py score_matrix 5 testdata_long.txt > testdata_long_aligned.fa     
...
```{bash}
echo -n "" > scores_exact.txt   # Create empty file to store exact scores
echo -n "" > scores_approx.txt  # Create empty file to store approx. scores
...
do
    ...
    python3 msa_sp_score_3k.py seq_triplet_aligned_exact.fa >> scores_exact.txt
    python3 msa_sp_score_3k.py seq_triplet_aligned_approx.fa >> scores_approx.txt

done
rm seq_triplet_aligned_exact.fa seq_triplet_aligned_approx.fa
```

Plotting the score results we can see:

```{r, eval=T, echo=FALSE}
scores_exact <- read.table("scores_exact.txt", header = F)
scores_approx <- read.table("scores_approx.txt", header = F)
